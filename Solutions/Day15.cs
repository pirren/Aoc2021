...
namespace Aoc2021.Solutions
{
    public class Day15 : DayBase
    {
        ...
        {
            var graph = GetGraph(indata);
            var map = new Map
            {
                Graph = graph,
                Width = indata.Trim().Split(Environment.NewLine)[0].Length,
                Height = indata.Trim().Split(Environment.NewLine).Length
            };
        ...
        {
            var graph = GetGraph(indata);
            var chunkWidth = indata.Trim().Split(Environment.NewLine)[0].Length;
            var chunkHeight = indata.Trim().Split(Environment.NewLine).Length;
        ...
        Dictionary<(int, int), int> GetGraph(string indata)
        {
            ...
            foreach (var (row, i) in indata.Trim().Split("\r\n").Select((row, idx) => (row, idx)))
            {
